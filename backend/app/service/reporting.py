...
1. 不要编造客户未填写的事实。
2. 不要改写评分数字和等级。
3. 输出包括管理摘要、三条关键短板解释、三到五个优先 AI 场景、下一步咨询建议。
4. 语气专业、务实，面向 CEO 和高管可读。
5. 不要输出邮件格式、致/发件人/主题、代码块、分隔线，也不要重复报告标题和总分卡片。
6. 必须解释 customer_classification 中的客户等级、当前诉求和建议打法；如果是“闪电战”，突出快速试点；如果是“攻坚战”，突出基础补齐；如果是“升维战”，突出规模化升级。
7. 使用清晰小标题和自然段即可。
